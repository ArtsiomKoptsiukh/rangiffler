...
`query.graphqls` - он должен использовать бэкендом, куда будут прилетать все запросы с фронта (назовем его здесь и далее, условно, `rangiffler-api`).
...
```posh
Dmitriis-MacBook-Pro rangiffler % bash localenv.sh
Dmitriis-MacBook-Pro rangiffler-client % cd rangiffler-auth
Dmitriis-MacBook-Pro rangiffler-client % gradle bootRun
```
...
#### 2. Запустите Wiremock, он заменит собо потенциальный сервис rangiffler-api, который только предстоит написать
...
#### 2. Далее, необходимо реализовать простейшую монолитную реализацию сервиса rangiffler-api
...
- Модели для описанных в graphql схеме типов (кроме типов Connection, подробности ниже)
- Контроллеры для query и mutation. Посмотри схему и вайрмоковские моки
